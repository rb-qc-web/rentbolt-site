...
const CACHE_KEY = "rentbolt:buildings:v14";
...
function parseBuilding(item, board) {
  ...
  (item.subitems || []).forEach(sub => {
    ...
    if (subLower === "rb website") {
      const galleryCol = sub.column_values?.find(cv => cv.id === "long_text5");
      if (galleryCol?.text) {
        try {
          const parsed = JSON.parse(galleryCol.text);
          if (Array.isArray(parsed) && parsed[0]?.includes("imagedelivery.net")) {
            photoGallery = parsed;
            photoUrl = parsed[0];
          }
        } catch {}
