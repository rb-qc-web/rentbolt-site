...
async function mondayQuery(query, variables = {}) {
  ...
  const res = await fetch(MONDAY_API_URL, {
    ...
    next: { revalidate: 900 }, // Cache for 15 min at the data layer
  });
...
function parseBuilding(item, city, region) {
  ...
  for (const col of item.column_values) {
    ...
    colsRaw[col.id] = col.value || "";
  }

  // Parse location from raw `value` field which contains lat/lng JSON
  let lat = 0, lng = 0;
  let address = cols[COLUMN_MAP.location] || "";
  const locVal = colsRaw[COLUMN_MAP.location];
  if (locVal) {
    try {
  ...
  const amenities = amenStr ? amenStr.split(",").map(s => s.trim()).filter(Boolean) : [];
  const amen = [];
  ...
  let tag = null;
  if (statusStr.includes("Lease-Up")) tag = "Available Now";
  else if (statusStr.includes("Stabilization")) tag = "Featured";

  const photoUrl = cols[COLUMN_MAP.photos] || "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=800&h=500&fit=crop";
  ...
  return {
    ...
    photo: photoUrl,
    photoUrl,
    petPolicy: cols[COLUMN_MAP.pets] || "",
    parkingInfo: cols[COLUMN_MAP.parking] || "",
    applicationProcess: "",
    tag,
  };
}
  ...
  try {
    for (const [key, board] of Object.entries(BOARDS)) {
      ...
      const buildings = items
        ...
        .filter(b => b.lat !== 0 && b.lng !== 0); // Only require valid location; price optional
